<script lang="ts">
  ...
  const fetchUptimeStatus = async () => {
    try {
      const [heartbeatRes, statusPageRes] = await Promise.all([
        fetchWithTimeout(HEARTBEAT_API_URL),
        fetchWithTimeout(STATUS_PAGE_URL),
      ]);

      if (!heartbeatRes.ok || !statusPageRes.ok) {
        throw new Error("Uptime API response is not ok");
      }
      ...
      let totalServices = 0;
      let upServices = 0;
      const newFailedServices: Array<{ name: string; statusText: string }> = [];
      ...
      failedServices = newFailedServices;
    } catch (e) {
      console.error("Failed to fetch Kuma status:", e);
      statusCode = -1;
      allServicesStatus = "获取状态失败";
      failedServices = [];
    }
  };
  ...
  $: config = statusConfig[statusCode.toString() as keyof typeof statusConfig];
  ...
  <div class={`overflow-hidden rounded-2xl border shadow-[inset_0_1px_0_rgba(255,255,255,0.2)] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] ${config?.bg} ${config?.border}`}>
    ...
    >
      <div class="flex min-w-0 items-center gap-2.5">
        <img src={uptimeIconUrl} alt="Uptime Icon" class="h-6 w-6 shrink-0" />
      ...
      <div class="relative z-[1] flex h-2.5 w-2.5 shrink-0">
        {#if config?.ping}
          <span class={`animate-ping absolute inline-flex h-full w-full rounded-full ${config?.pingColor} opacity-70`}></span>
        {/if}
